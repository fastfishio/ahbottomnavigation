		...
		if (mMenu != null) {
			for (int i = 0; i < mMenu.size(); i++) {
				...
				if (colors != null && colors.length >= mMenu.size() && colors[i] != 0) {
					AHBottomNavigationItem navigationItem = new AHBottomNavigationItem(String.valueOf(item.getTitle()), item.getIcon(), colors[i], null, i == 0 ? true : false, -16);
					navigationItems.add(navigationItem);
				} else {
					AHBottomNavigationItem navigationItem = new AHBottomNavigationItem(String.valueOf(item.getTitle()), item.getIcon(), null, i == 0 ? true : false, -16);
					navigationItems.add(navigationItem);
				}
			}
